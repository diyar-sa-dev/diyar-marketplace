...
async function ensureCustomerAddress(request: import('@playwright/test').APIRequestContext): Promise<string> {
  ...
  const created = await request.post(`${apiBaseUrl()}/profile/addresses`, {
    headers: { ...headers, 'Content-Type': 'application/json' },
    data: {
      label: 'E2E Home',
      type: 'home',
      recipient_name: demoUsers.customer.name,
      phone: demoUsers.customer.phoneNational,
      city: 'Riyadh',
      district: 'Al Olaya',
      street: 'Test St',
      building: '1',
...
test.describe('Checkout journey', () => {
  ...
  test('API checkout and fake payment completes order', async ({ request }) => {
    ...
    const productId = (await products.json())?.data?.items?.[0]?.id;
  ...
  test('UI cart to fake payment simulator', async ({ page, request }) => {
    ...
    expect(orderRes.ok()).toBeTruthy();
    const orderId = (await orderRes.json())?.data?.order?.id as string;
