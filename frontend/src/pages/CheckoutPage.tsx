...
const PAYMENT_METHODS = [
  { id: 'mada', name: 'مدى', logo: '/payment-methods/Mada_Logo.svg', type: 'image' },
  {
    ...
    logo: <CreditCard className="w-8 h-8 text-gray-700" />,
    type: 'icon',
  },
...
export default function CheckoutPage() {
  ...
  const applyCoupon = (vendorId: string) => {
    ...
    if (vendorCoupons[vendorId] === 'DISCOUNT10') {
      const updatedCart = cart.map((v) => (v.vendorId === vendorId ? { ...v, discount: 50 } : v));
      setCart(updatedCart);
  ...
  const totalAssembly = cart.reduce(
    (total, vendor) =>
      total +
      vendor.items.reduce(
        (sum, item) => (assemblySelections[item.id] ? sum + vendor.assemblyCostPerItem : sum),
        0,
      ),
    0,
  );
  ...
  const finalTotal =
    subtotal + totalShipping + totalAssembly - totalDiscount - autoOfferDiscount + vat;
              ...
              {MOCK_ADDRESSES.map((addr) => (
                ...
                >
                  <div className="flex justify-between items-start mb-2">
                    ...
                    {selectedAddress === addr.id && (
                      <CheckCircle className="text-diyar-brown w-5 h-5" />
                    )}
                  </div>
          ...
          <div className="bg-white rounded-3xl p-6 shadow-sm border border-gray-100">
            ...
            <div className="space-y-8">
              {cart.map((vendor) => (
                <div
                  key={vendor.vendorId}
                  className="border border-gray-100 rounded-2xl p-4 md:p-6 bg-gray-50/50"
                >
                          ...
                          {item.requiresAssembly && (
                            <label className="flex items-center gap-2 mt-2 p-2 bg-blue-50/50 rounded-lg border border-blue-100 cursor-pointer w-fit">
                              ...
                              <span className="text-xs font-medium text-blue-900 flex items-center gap-1">
                                <Wrench size={12} /> طلب تنفيذ التركيب (+
                                {vendor.assemblyCostPerItem} ر.س)
                              </span>
                            </label>
                          )}
                        </div>
                      ...
                      <div className="relative h-full flex items-center justify-center w-full">
                        <img
                          ...
                          onError={(e) => {
                            ...
                            const fallback =
                              target.parentElement?.querySelector('.payment-fallback');
          ...
          <div className="bg-white rounded-3xl p-6 shadow-sm border border-gray-100 sticky top-24">
            ...
            <div className="flex justify-between items-end mb-8">
              ...
              <div className="text-right">
                <span className="font-bold text-2xl md:text-3xl text-diyar-dark">
