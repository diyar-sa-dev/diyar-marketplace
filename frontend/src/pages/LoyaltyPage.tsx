  ...
  const history = [
    {
      ...
      points: 150,
      date: '10 مايو 2026',
      desc: 'مكافأة شراء: طقم كنب مودرن',
      amount: '1500 ر.س',
    },
    {
      id: 2,
    ...
    },
    {
      ...
      points: 100,
      date: '28 أبريل 2026',
      desc: 'استبدال نقاط في متجر مفروشات الرقي',
      amount: '-100 ر.س',
    },
    {
      id: 4,
      ...
      <div className="max-w-4xl mx-auto px-4">
        <div className="text-center mb-8 md:mb-12">
          ...
          <p className="text-gray-500 text-lg">
            مكافآت حصرية وعروض مميزة بانتظارك مع كل عملية شراء.
          </p>
        </div>
          ...
          <div className="relative z-10 flex flex-col md:flex-row items-center justify-between gap-8">
            ...
            <div className="bg-white/10 p-5 rounded-2xl backdrop-blur-md border border-white/20 text-center w-full md:w-auto">
              ...
              <span className="text-3xl font-bold font-sans">
                200 <span className="text-lg font-normal">ر.س</span>
              </span>
            </div>
          </div>
        </div>
        ...
        <div className="bg-white rounded-3xl shadow-sm border border-gray-100 overflow-hidden">
          <div className="p-6 md:p-8 border-b border-gray-100 flex flex-col md:flex-row items-center justify-between gap-4">
            ...
            <div className="flex items-center p-1 bg-gray-50 rounded-xl w-full md:w-auto">
              <button
                ...
                className={`flex-1 md:flex-none px-6 py-2 rounded-lg text-sm font-bold transition-all ${activeTab === 'all' ? 'bg-white shadow-sm text-diyar-dark' : 'text-gray-500 hover:text-diyar-dark'}`}
              >
                الكل
              </button>
              <button
                onClick={() => setActiveTab('earned')}
                className={`flex-1 md:flex-none px-6 py-2 rounded-lg text-sm font-bold transition-all ${activeTab === 'earned' ? 'bg-white shadow-sm text-green-600' : 'text-gray-500 hover:text-green-600'}`}
              >
                اكتساب
              </button>
              <button
                onClick={() => setActiveTab('used')}
                className={`flex-1 md:flex-none px-6 py-2 rounded-lg text-sm font-bold transition-all ${activeTab === 'used' ? 'bg-white shadow-sm text-red-600' : 'text-gray-500 hover:text-red-600'}`}
              >
                استبدال
              </button>
            </div>
          ...
          <div className="divide-y divide-gray-100">
            {filteredHistory.map((item) => (
              ...
              >
                <div className="flex items-start gap-4">
                  <div
                    className={`w-12 h-12 rounded-full flex items-center justify-center shrink-0 ${item.type === 'earned' ? 'bg-green-50 text-green-600' : 'bg-red-50 text-red-600'}`}
                  >
                    {item.type === 'earned' ? (
                      <ArrowUpRight size={24} />
                    ) : (
                ...
                <div
                  className={`text-xl font-sans font-bold flex items-center gap-1 ${item.type === 'earned' ? 'text-green-600' : 'text-red-600'}`}
                >
                  {item.type === 'earned' ? '+' : '-'}
                  {item.points}
            ...
            {filteredHistory.length === 0 && (
              <div className="p-12 text-center text-gray-500">لا توجد عمليات مطابقة للبحث</div>
            )}
          </div>
        </div>
      </div>
