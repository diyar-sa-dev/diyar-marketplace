...
export default function ServicePage() {
  ...
  const MOCK_REVIEWS = [
    {
      ...
      rating: 5,
      date: 'قبل يومين',
      comment: 'خدمة ممتازة وتصميم رائع، التزموا بالمواعيد وبكل التفاصيل. أنصح بالتعامل معهم بشدة.',
      avatar:
    ...
    },
    {
      ...
      rating: 4,
      date: 'قبل أسبوع',
      comment: 'العمل معهم كان مريحاً جداً، أفكارهم عصرية وحلولهم عملية. شكرا إيوان.',
      avatar:
    ...
    },
    {
      ...
      rating: 5,
      date: 'قبل شهر',
      comment: 'افضل مصممين تعاملت معهم، حولوا مساحتي لمكان خيالي.',
      avatar:
  ...
  const RELATED_SERVICES = [
    {
      id: 2,
      name: 'تنفيذ وتشطيب الديكور',
      vendor: 'إيوان للتصميم',
      rating: 4.8,
      type: 'زيارة معاينة',
      price: '٤٥٠',
      img: 'https://images.unsplash.com/photo-1581858726788-75bc0f6a952d?auto=format&fit=crop&q=80&w=600',
    },
    {
      id: 3,
      name: 'تركيب الأثاث والمطابخ',
      vendor: 'فن الإنجاز',
      rating: 4.7,
      type: 'تحديد موعد',
      price: '٢٠٠',
      img: 'https://images.unsplash.com/photo-1556909212-d5b604d0c90d?auto=format&fit=crop&q=80&w=600',
    },
    {
      id: 4,
      name: 'دهانات وديكورات جبسية',
      vendor: 'لمسات فنية',
      rating: 4.9,
      type: 'زيارة معاينة',
      price: '٣٥٠',
    ...
    {
      id: 5,
      name: 'تفصيل وتركيب الستائر',
      vendor: 'بيت الستائر',
      rating: 4.6,
      type: 'تحديد موعد',
      price: '٣٠٠',
      img: 'https://images.unsplash.com/photo-1513161455079-7dc1de15ef3e?auto=format&fit=crop&q=80&w=600',
    },
  ];
          ...
          <div className="md:col-span-2 space-y-8">
            <div className="bg-white rounded-xl md:rounded-2xl border border-gray-100 p-6 shadow-sm">
              ...
              <p className="text-gray-600 leading-relaxed mb-6">
                {SERVICE_INFO.description}
                الخدمة تشمل الآتي: دراسة المبنى، تحديد الطراز المناسب، توزيع الأثاث والتوزيع الضوئي،
                عمل المخططات التنفيذية، وإخراج المناظير ثلاثية الأبعاد للتصميم.
              </p>
            ...
            <div className="bg-white rounded-xl md:rounded-2xl border border-gray-100 p-6 shadow-sm">
                  ...
                  <div className="flex gap-1">
                    {[1, 2, 3, 4, 5].map((star) => (
                      <Star
                        key={star}
                        size={16}
                        className={`${star <= Math.floor(SERVICE_INFO.rating) ? 'fill-amber-400 text-amber-400' : 'text-gray-300'}`}
                      />
                    ))}
                  </div>
                  <span className="text-sm text-gray-500 mt-1">
                    بناءً على {SERVICE_INFO.reviews} تقييم
                  </span>
                </div>
              </div>
              ...
              <div className="space-y-4">
                {MOCK_REVIEWS.map((review) => (
                  <div
                    key={review.id}
                    className="p-4 rounded-xl border border-gray-50 bg-gray-50/50 hover:bg-gray-50 transition-colors"
                  >
        ...
        <div className="fixed inset-0 bg-black/95 z-[200] flex flex-col justify-center animate-in fade-in duration-300">
          ...
          <div className="relative w-full max-w-5xl mx-auto p-4 md:p-12">
            <div className="aspect-[4/3] md:aspect-video rounded-2xl overflow-hidden shadow-2xl relative bg-black flex items-center justify-center">
              <img
                src={
                  SERVICE_INFO.gallery[galleryIndex % SERVICE_INFO.gallery.length] ||
                  SERVICE_INFO.cover
                }
                alt="Gallery"
            ...
            <div className="absolute inset-y-0 right-4 md:right-8 flex items-center">
              <button
                onClick={() =>
                  setGalleryIndex(
                    (prev) =>
                      (prev - 1 + SERVICE_INFO.gallery.length) % SERVICE_INFO.gallery.length,
                  )
                }
                className="w-12 h-12 bg-white/10 hover:bg-white/20 backdrop-blur-md rounded-full flex items-center justify-center text-white transition"
              >
