...
export default function SearchPage() {
  ...
  const allStores = [
    ...
    },
    {
      id: 2,
      name: 'أشلي',
      type: 'أثاث كلاسيكي ومودرن',
      logo: 'https://images.unsplash.com/photo-1594026112284-02bb6f3352fe?auto=format&fit=crop&q=80&w=200',
      rating: 4.6,
    },
    {
      id: 3,
      name: 'مفروشات الرقي',
      type: 'أثاث محلي فاخر',
      logo: 'https://images.unsplash.com/photo-1583847268964-b28ce8f30e9b?auto=format&fit=crop&q=80&w=200',
      rating: 4.9,
    },
  ];
  ...
  let filteredProducts =
    searchQuery && !isVisualSearch
      ? allProducts.filter((p) => p.name.includes(searchQuery) || p.store.includes(searchQuery))
      : allProducts;
  let filteredStores =
    searchQuery && !isVisualSearch
      ? allStores.filter((s) => s.name.includes(searchQuery) || s.type.includes(searchQuery))
      : allStores;
  let filteredServices =
    searchQuery && !isVisualSearch
      ? allServices.filter((s) => s.name.includes(searchQuery) || s.provider.includes(searchQuery))
      : allServices;
        ...
        <div className="mb-6">
          ...
          {hasExactMatches ? (
            <p className="text-gray-500 text-sm">
              وجدنا {totalResults} نتيجة مطابقة {isVisualSearch ? 'للصورة' : 'لبحثك'}
            </p>
          ) : (
            <p className="text-amber-600 bg-amber-50 px-3 py-1.5 rounded-lg inline-block text-xs">
              لم نجد نتائج مطابقة تماماً {isVisualSearch ? 'للصورة المرفوعة' : 'لبحثك'}، لكننا نقترح
              عليك هذه النتائج المميزة
            </p>
          )}
        ...
        {totalResults === 0 ? (
          <div className="bg-white rounded-3xl p-16 text-center shadow-sm border border-gray-100 flex flex-col items-center">
            ...
            <p className="text-gray-500 mb-8 max-w-md">
              نعتذر، لم نتمكن من العثور على ما تبحث عنه. يرجى التحقق من الإملاء أو تجربة كلمات
              مفتاحية مختلفة.
            </p>
            <Link
              to="/"
                ...
                <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-4">
                  {filteredStores.map((store) => (
                    <Link
                      to={`/store/${store.id}`}
                      key={store.id}
                      className="bg-white p-3 rounded-xl shadow-sm border border-gray-100 flex items-center gap-3 hover:shadow-md transition-shadow group"
                    >
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4">
                  {filteredServices.map((service) => (
                    <Link
                      ...
                      key={service.id}
                      className="min-w-[280px] md:min-w-0 bg-white rounded-xl shadow-sm border border-gray-100 flex flex-col overflow-hidden hover:-translate-y-1 hover:shadow-md transition-all snap-start group"
                    >
                      ...
                      <div className="p-4 flex flex-col flex-1">
                        <h3 className="font-bold text-diyar-dark text-base mb-1 truncate">
                          {service.name}
                        </h3>
                        ...
                        <div className="flex items-center justify-between mt-auto pt-3 border-t border-gray-50">
                          <span className="font-bold text-diyar-brown text-sm">
                            {service.price}
                          </span>
