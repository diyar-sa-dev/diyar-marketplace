    ...
    subOrders: [
      {
        ...
        items: [
          {
            name: 'وسادة ديكور مطرزة',
            img: 'https://images.unsplash.com/photo-1584043960100-c831e78ebfd3?auto=format&fit=crop&q=80&w=200',
            isReviewed: true,
          },
        ],
      },
    ...
    <div className="min-h-screen bg-gray-50 pt-20 pb-24">
      <div className="bg-diyar-dark text-white py-8 mb-8">
        <div className="max-w-7xl mx-auto px-4">
          ...
          <p className="text-diyar-cream/80 text-sm">
            تتبع شحناتك، راجع تفاصيل الطلب، وقيّم المنتجات المستلمة.
          </p>
        </div>
      </div>
                  ...
                  <div key={i} className="border border-gray-100 rounded-2xl p-5 relative">
                    ...
                    <div className="mb-6 mt-2">
                      ...
                      <div className="relative mt-6 mb-4 px-4">
                        ...
                        <div
                          className="absolute top-1/2 right-4 h-1 bg-green-500 -translate-y-1/2 rounded-full -z-10 transition-all duration-1000"
                          style={{
                            width:
                              subOrder.statusStep === 1
                                ? '0%'
                                : subOrder.statusStep === 2
                                  ? '50%'
                                  : '100%',
                          }}
                        ></div>
                        ...
                        <div className="flex justify-between px-2">
                          <div
                            className={`w-6 h-6 rounded-full flex items-center justify-center ${subOrder.statusStep >= 1 ? 'bg-green-500 text-white' : 'bg-gray-200 text-gray-400'}`}
                          >
                            <Package size={12} />
                          </div>
                          <div
                            className={`w-6 h-6 rounded-full flex items-center justify-center ${subOrder.statusStep >= 2 ? 'bg-green-500 text-white' : 'bg-gray-200 text-gray-400'}`}
                          >
                            <Truck size={12} />
                          </div>
                          <div
                            className={`w-6 h-6 rounded-full flex items-center justify-center ${subOrder.statusStep >= 3 ? 'bg-green-500 text-white' : 'bg-gray-200 text-gray-400'}`}
                          >
                            <CheckCircle size={12} />
                          </div>
                        </div>
                    ...
                    <div className="space-y-3 bg-gray-50/50 rounded-xl p-3">
                      {subOrder.items.map((item, j) => (
                        <div
                          key={j}
                          className="flex gap-4 items-center bg-white p-2 rounded-lg border border-gray-100 shadow-sm"
                        >
                          <img
                            src={item.img}
                            alt={item.name}
                            ...
                            <button
                              ...
                              className={`text-xs px-3 py-1.5 rounded-lg font-bold transition-colors ${
                                item.isReviewed
                                  ...
                                  : 'bg-diyar-cream text-diyar-brown hover:bg-diyar-brown hover:text-white border border-diyar-brown/20'
                              }`}
            ...
            <div className="flex justify-center gap-2 mb-6">
              {[1, 2, 3, 4, 5].map((star) => (
                <Star
                  key={star}
                  size={32}
                  className="text-gray-200 hover:text-yellow-400 cursor-pointer transition-colors"
                />
              ))}
            </div>
