  ...
  const handleSend = () => {
    ...
    setTimeout(() => {
      ...
      const newAIMessage: Message = {
        ...
        type: 'assistant',
        content:
          'فهمت ما تبحث عنه. بناءً على وصفك، أقترح ألواناً دافئة مثل البيج الترابي مع لمسات من الخشب الطبيعي. هذه بعض القطع التي قد تعجبك:',
        products: [
          {
            name: 'كنبة لوسيا ٣ مقاعد',
            image:
          ...
          },
          {
            ...
            image:
              'https://images.unsplash.com/photo-1533090368676-1fd25485db88?auto=format&fit=crop&q=80&w=200',
            price: '850',
          },
        ],
      };
  ...
  const handleImageUpload = (e: React.ChangeEvent<HTMLInputElement>) => {
    if (e.target.files && e.target.files[0]) {
      ...
      const newUserMessage: Message = {
        ...
        content:
          document.dir === 'rtl'
            ? 'ابحث لي عن أثاث مشابه لهذه الصورة'
            : 'Find furniture similar to this image',
        image: imageUrl,
      };
      ...
      setTimeout(() => {
        ...
        const newAIMessage: Message = {
          ...
          type: 'assistant',
          content:
            'لقد قمت بتحليل الصورة. النمط المعروض هو "مودرن مينيماليست" ويتميز بالخطوط النظيفة والألوان المحايدة. وجدت بعض القطع المطابقة في متجرنا:',
          products: [
            {
              name: 'كرسي استرخاء مفرد',
              image:
                'https://images.unsplash.com/photo-1598300042247-d088f8ab3a91?auto=format&fit=crop&q=80&w=200',
              price: '1,200',
            },
            {
              name: 'مصباح أرضي مقوس',
              image:
      ...
      <div className="flex-1 overflow-y-auto p-4 md:p-6 pb-4 space-y-6 scroll-smooth">
        {messages.map((message) => (
          ...
          >
            {/* Avatar */}
            <div
              className={`w-10 h-10 rounded-full flex items-center justify-center shrink-0 ${message.type === 'user' ? 'bg-gray-200 text-gray-600' : 'bg-diyar-brown text-white'}`}
            >
            ...
            {/* Content */}
            <div
              className={`max-w-[85%] md:max-w-[75%] ${message.type === 'user' ? 'items-end text-right' : 'items-start text-right'} flex flex-col gap-2`}
            >
              {message.image && (
              ...
              <div
                className={`px-5 py-3.5 rounded-2xl text-sm md:text-base leading-relaxed ${
                  message.type === 'user'
              ...
              {message.products && (
                <div className="grid grid-cols-2 md:grid-cols-3 gap-3 mt-2 w-full">
                  {message.products.map((product, idx) => (
                    <div
                      key={idx}
                      className="bg-white rounded-xl border border-gray-100 p-2 shadow-sm flex flex-col gap-2 hover:border-diyar-brown transition-colors cursor-pointer group"
                    >
                      ...
                      <div>
                        <h4 className="text-xs font-bold text-diyar-dark line-clamp-1">
                          {product.name}
                        </h4>
                        <p className="text-xs text-diyar-brown font-bold mt-1">
                          {product.price} ر.س
                        </p>
                      </div>
                    </div>
