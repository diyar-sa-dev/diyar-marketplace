...
export default function VendorTeam() {
  ...
  const getRoleBadge = (role: string) => {
    switch (role) {
      case 'owner':
        return (
          <span className="bg-purple-100 text-purple-700 px-2.5 py-1 rounded-full text-xs font-bold flex items-center gap-1 w-fit">
            <Shield size={12} /> مالك المتجر
          </span>
        );
      case 'manager':
        return (
              ...
              <div>
                <h3 className="font-bold text-diyar-dark text-lg flex items-center gap-2">
                  ...
                  {member.status === 'invited' && (
                    <span className="text-xs bg-amber-50 text-amber-600 px-2 py-0.5 rounded border border-amber-100 font-normal">
                      دعوة معلقة
                    </span>
                  )}
                </h3>
            ...
            <div className="flex items-center justify-between md:justify-end gap-6 md:gap-8 border-t border-gray-100 md:border-t-0 pt-4 md:pt-0 mt-2 md:mt-0">
              ...
              <div className="flex gap-2">
                {member.role !== 'owner' && (
                  <>
                    <button
                      className="p-2 text-gray-400 hover:text-blue-500 transition bg-gray-50 rounded-lg hover:bg-blue-50"
                      title="تعديل الصلاحية"
                    >
                      <Edit size={18} />
                    </button>
                    <button
                      className="p-2 text-gray-400 hover:text-red-500 transition bg-gray-50 rounded-lg hover:bg-red-50"
                      title="حذف"
                    >
                      <Trash2 size={18} />
                    </button>
        ...
        <div className="fixed inset-0 bg-black/60 z-[300] flex items-center justify-center p-4 animate-in fade-in duration-300">
          <div className="bg-white rounded-2xl md:rounded-3xl shadow-2xl w-full max-w-md flex flex-col overflow-hidden animate-in zoom-in-95 duration-300">
            ...
            <div className="p-6 space-y-4">
              <div className="space-y-2">
                ...
                <input
                  ...
                  className="w-full p-2.5 border border-gray-200 rounded-lg focus:outline-none focus:border-diyar-brown"
                  dir="ltr"
                />
              </div>
              <div className="space-y-2">
            ...
            <div className="p-6 border-t border-gray-100 flex justify-end gap-3 bg-gray-50 mt-4 shrink-0">
              ...
              <button
                onClick={() => setIsInviteModalOpen(false)}
                className="px-5 py-2.5 rounded-xl font-bold bg-diyar-brown text-white hover:bg-[#A67B5B]/90 transition"
              >
                إرسال الدعوة
              </button>
            </div>
