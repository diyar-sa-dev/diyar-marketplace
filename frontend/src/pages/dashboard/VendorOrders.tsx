...
export default function VendorOrders() {
  ...
  const getStatusBadge = (status: string) => {
    switch (status) {
      case 'pending':
        return (
          <span className="bg-amber-100 text-amber-700 px-2.5 py-1 rounded-full text-xs font-bold border border-amber-200">
            بانتظار التأكيد
          </span>
        );
      case 'processing':
        return (
          <span className="bg-blue-100 text-blue-700 px-2.5 py-1 rounded-full text-xs font-bold border border-blue-200">
            قيد التحضير
          </span>
        );
      case 'shipped':
        return (
          <span className="bg-purple-100 text-purple-700 px-2.5 py-1 rounded-full text-xs font-bold border border-purple-200">
            تم الشحن
          </span>
        );
      case 'delivered':
      ...
      case 'cancelled':
        return (
          <span className="bg-red-100 text-red-700 px-2.5 py-1 rounded-full text-xs font-bold border border-red-200">
            ملغي
          </span>
        );
      default:
  ...
  const getFilteredOrders = () => {
    ...
    if (searchTerm) {
      orders = orders.filter(
        (order) => order.id.includes(searchTerm) || order.customer.includes(searchTerm),
      );
    }
    return orders;
  };
            ...
            <div className="relative group/actions">
              ...
              <div className="absolute left-0 top-full mt-2 w-48 bg-white border border-gray-100 rounded-xl shadow-lg opacity-0 invisible group-hover/actions:opacity-100 group-hover/actions:visible transition-all z-10 py-1">
                <button
                  ...
                  className="w-full text-right px-4 py-2 hover:bg-gray-50 text-sm flex items-center gap-2"
                >
                  <Package size={14} />
                  قيد التحضير
                </button>
            ...
            <div className="bg-white rounded-2xl border border-gray-100 shadow-sm overflow-hidden flex flex-col">
              ...
              <div className="divide-y divide-gray-100">
                {[...Array(selectedOrder.items)].map((_, i) => (
                  <div
                    key={i}
                    className="p-6 flex items-center gap-4 hover:bg-gray-50/30 transition-colors"
                  >
                    <div className="w-20 h-20 bg-gray-100 rounded-xl overflow-hidden shrink-0 border border-gray-100">
                      <img
                        src="https://images.unsplash.com/photo-1555041469-a586c61ea9bc?auto=format&fit=crop&q=80&w=150"
                        alt="Product"
                    ...
                    <div className="flex-1">
                      ...
                      <div className="text-sm text-gray-500 mb-2">
                        القسم: الصالونات • اللون: رمادي
                      </div>
                      <div className="flex items-center justify-between">
                        <span className="text-sm font-bold text-diyar-brown">
                          {(selectedOrder.total / selectedOrder.items).toFixed(0)} ر.س
                        </span>
                        <span className="text-sm bg-gray-100 px-2.5 py-1 rounded-lg">
                          الكمية: 1
                        </span>
                      </div>
                    </div>
            ...
            <div className="bg-white rounded-2xl border border-gray-100 shadow-sm p-6">
              ...
              <div className="relative pl-4 space-y-6 before:absolute before:inset-y-0 before:right-[11px] before:w-0.5 before:bg-gray-100 w-fit">
                <div className="relative pl-6">
                  <div
                    className={`absolute right-[-14px] top-1 w-4 h-4 rounded-full border-4 border-white ${selectedOrder.status === 'delivered' ? 'bg-green-500' : 'bg-gray-200'}`}
                  ></div>
                  <h4
                    className={`font-bold ${selectedOrder.status === 'delivered' ? 'text-green-600' : 'text-gray-400'}`}
                  >
                    تم التسليم
                  </h4>
                  {selectedOrder.status === 'delivered' && (
                    <p className="text-xs text-gray-500 mt-1">تم التسليم بنجاح للعميل</p>
                  )}
                </div>
                <div className="relative pl-6">
                  <div
                    className={`absolute right-[-14px] top-1 w-4 h-4 rounded-full border-4 border-white ${['shipped', 'delivered'].includes(selectedOrder.status) ? 'bg-purple-500' : 'bg-gray-200'}`}
                  ></div>
                  <h4
                    className={`font-bold ${['shipped', 'delivered'].includes(selectedOrder.status) ? 'text-purple-600' : 'text-gray-400'}`}
                  >
                    تم الشحن
                  </h4>
                  {['shipped', 'delivered'].includes(selectedOrder.status) && (
                    <p className="text-xs text-gray-500 mt-1">
                      الطلب مع شركة الشحن (أرامكس - #43422312)
                    </p>
                  )}
                </div>
                <div className="relative pl-6">
                  <div
                    className={`absolute right-[-14px] top-1 w-4 h-4 rounded-full border-4 border-white ${['processing', 'shipped', 'delivered'].includes(selectedOrder.status) ? 'bg-blue-500' : 'bg-gray-200'}`}
                  ></div>
                  <h4
                    className={`font-bold ${['processing', 'shipped', 'delivered'].includes(selectedOrder.status) ? 'text-blue-600' : 'text-gray-400'}`}
                  >
                    قيد التحضير
                  </h4>
                  {['processing', 'shipped', 'delivered'].includes(selectedOrder.status) && (
                    <p className="text-xs text-gray-500 mt-1">جاري تجهيز وتغليف المنتجات</p>
                  )}
                </div>
                <div className="relative pl-6">
                  ...
                  <p className="text-xs text-gray-500 mt-1">
                    تم تأكيد الدفع واستلام الطلب ({selectedOrder.date})
                  </p>
                </div>
              </div>
            </div>
            ...
            <div className="bg-white rounded-2xl border border-gray-100 shadow-sm p-6">
              ...
              <div className="space-y-4">
                ...
                <div>
                  ...
                  <span
                    className={`text-sm px-3 py-1.5 rounded-lg border font-bold ${selectedOrder.payment === 'paid' ? 'bg-green-50 text-green-700 border-green-200' : selectedOrder.payment === 'refunded' ? 'bg-gray-50 text-gray-700 border-gray-200' : 'bg-red-50 text-red-700 border-red-200'}`}
                  >
                    {selectedOrder.payment === 'paid'
                      ? 'مدفوع إلكترونياً (فيزا)'
            ...
            <div className="bg-white rounded-2xl border border-gray-100 shadow-sm p-6">
              ...
              <div className="space-y-2">
                ...
                <p className="text-sm text-gray-600 leading-relaxed">
                  ...
                  <br />
                  حي الملقا، شارع الأمير محمد بن سعد
                  <br />
                  مبنى رقم 45، شقة 12
                </p>
  ...
  return (
    <div className="space-y-6">
      ...
      {isAddModalOpen && (
        <div
          className="fixed inset-0 bg-black/60 backdrop-blur-sm z-50 flex items-center justify-center p-4 animate-in fade-in duration-250"
          dir="rtl"
        >
      ...
      <div className="bg-white border md:rounded-2xl border-gray-100 shadow-sm overflow-hidden border-x-0 md:border-x">
              ...
              {getFilteredOrders().map((order) => (
                <tr key={order.id} className="hover:bg-gray-50/50 transition-colors">
                  ...
                  <td className="px-6 py-4">
                    <span
                      className={`text-xs px-2 py-1 rounded-md ${order.payment === 'paid' ? 'bg-green-50 text-green-600' : order.payment === 'refunded' ? 'bg-gray-100 text-gray-600' : 'bg-red-50 text-red-600'}`}
                    >
                      {order.payment === 'paid'
                        ? 'مدفوع'
                  ...
                  <td className="px-6 py-4">
                    <div className="flex gap-2">
                      <button
                        ...
                        className="text-gray-400 hover:text-diyar-brown transition p-1"
                        title="عرض التفاصيل"
                      >
                        <Eye size={18} />
                      </button>
        ...
        {getFilteredOrders().length === 0 && (
          <div className="text-center py-12 text-gray-500">لا توجد طلبات تطابق معايير البحث</div>
        )}
      </div>
    </div>
  );
