...
export default function Notifications() {
  const notifications = [
    ...
    },
    {
      ...
      message:
        'تم تحديث سياسات التسعير، يرجى مراجعة الشروط والأحكام الجديدة لتجنب أي مشاكل في حسابك.',
      time: 'منذ يوم',
      read: true,
    },
    {
    ...
    <div className="space-y-6 animate-in fade-in duration-300">
      ...
      <div className="bg-white rounded-2xl border border-gray-100 shadow-sm overflow-hidden">
        {notifications.map((notif) => (
          <div
            key={notif.id}
            className={`p-6 border-b border-gray-50 hover:bg-gray-50 transition cursor-pointer flex items-start gap-4 ${!notif.read ? 'bg-amber-50/20' : ''}`}
          >
            <div
              className={`w-12 h-12 rounded-full shrink-0 flex items-center justify-center ${notif.type === 'order' ? 'bg-amber-100 text-amber-600' : 'bg-blue-100 text-blue-600'}`}
            >
            ...
            <div className="flex-1">
              <div className="flex items-start justify-between mb-1">
                <h3
                  className={`font-bold text-lg ${!notif.read ? 'text-diyar-dark' : 'text-gray-700'}`}
                >
                  {notif.title}
                </h3>
              ...
              <p
                className={`text-sm ${!notif.read ? 'text-gray-700 font-medium' : 'text-gray-500'}`}
              >
                {notif.message}
              </p>
            </div>
