...
export function normalizeCatalogSearchFilters(
  ...
  const minPrice =
    raw.min_price !== undefined && raw.min_price !== '' ? Number(raw.min_price) : undefined;
  const maxPrice =
    raw.max_price !== undefined && raw.max_price !== '' ? Number(raw.max_price) : undefined;

  const normalizedQuery =
    raw.q !== undefined && raw.q !== null ? String(raw.q).replace(/\s+/g, ' ').trim() : '';
  const colors = (() => {
    if (raw.colors) {
