...
final class VendorAnalyticsService
{
    ...
    {
        $currency = (string) config('diyar.finance.currency', 'SAR');
        $buckets = $this->buildBuckets($range['from'], $range['to'], $range['granularity']);
        $series = [];
     ...
     */
    private function buildBuckets(CarbonImmutable $from, CarbonImmutable $to, string $granularity): array
    {
        return match ($granularity) {
            'hour' => $this->hourlyBuckets($from, $to),
            'week' => $this->weeklyBuckets($from, $to),
            'month' => $this->monthlyBuckets($from, $to),
            default => $this->dailyBuckets($from, $to),
        };
    }
     ...
     * @return list<array{label: string, from: CarbonImmutable, to: CarbonImmutable}>
     */
    private function weeklyBuckets(CarbonImmutable $from, CarbonImmutable $to): array
    {
        $buckets = [];
        $cursor = $from->startOfDay();
