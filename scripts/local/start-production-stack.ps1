...
Write-Host ''
Write-Host 'Full stack (one command — Docker + Vite + gateway):'
Write-Host '  .\scripts\local\start-local-dev.ps1'
Write-Host ''
Write-Host 'Or manually (3 terminals):'
Write-Host '  .\scripts\local\start-frontend-prod-api.ps1   # Vite (React UI)'
Write-Host '  .\scripts\local\start-dev-gateway.ps1         # nginx on :8080'
Write-Host ''
Write-Host "Stable browser URL: $($network.GatewayUrl)"
Write-Host '  (Docker alone is API-only — you need Vite for the React storefront)'
